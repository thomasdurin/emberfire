...
export default class FirestoreAdapter extends DS.Adapter.extend({
  ...
  query<K extends keyof ModelRegistry>(
    ...
    return rootCollection(this, type)
      .then(collection => queryDocs(collection, queryOptionsToQueryFn(options)))
...
const queryOptionsToQueryFn = (options?: QueryOptions) => (
    ...
    return Promise.all(
      relationshipsToInclude.map(r => {
        if (r.meta.kind == "hasMany") {
          ...
          const belongsToIds = [
            ...new Set(
              collection.docs.map(d => d.data()[r.meta.key]).filter(id => !!id)
            )
          ];
          return Promise.all(
            belongsToIds.map(id =>
